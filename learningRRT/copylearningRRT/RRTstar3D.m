...
figure;
path_400 = findMinimumPath(tree_400,end_node,dim);
plotExpandedTree(world,tree_400,dim);
plotWorld(world,path_400,dim);

% figure;
% path_800 = findMinimumPath(tree_800,end_node,dim);
% plotExpandedTree(world,tree_800,dim);
        ...
        for j = 1:size(reduced_idx,1)
          ...
          rnewcost = min_cost + lcost;
            if near_cost > rnewcost
                ...
                if  collision_flag==0 && norm(trajend-new_tree(reduced_idx(j),1:2*dim))<0.5
           
                ecost = rnewcost - near_cost;                
%                 before = new_tree(reduced_idx(j),2*dim+3)
...
%                 after = new_tree(reduced_idx(j),2*dim+3)
