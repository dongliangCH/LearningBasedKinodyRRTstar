...
%     plot(path(i+1,1),path(i+1,2),'o','LineWidth',2,'MarkerSize',8);hold on
    ...
    z0=[path(i,1:dim)-path(i+1,1:dim),path(i,dim+1:2*dim)]';
    zf=path(i+1,dim+1:2*dim)'; 
    Tf=T202020([z0;zf]);
    dt = 0.02;
    Horizon = Tf/dt;
    t = linspace(0,Tf,Horizon);
    ...
    traj=[z1(1:dim,:)+path(i+1,1:dim)'*ones(1,size(z1,2));z1(dim+1:2*dim,:)];
    plot3(traj(1,1:size(t,2)),traj(2,1:size(t,2)),traj(3,1:size(t,2)),'color','r','LineWidth',2);hold on
    xl = xlabel('$x (m)$','Interpreter','LaTeX');
    yl = ylabel('$y (m)$','Interpreter','LaTeX');
