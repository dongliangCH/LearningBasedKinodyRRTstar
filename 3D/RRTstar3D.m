...
function [new_tree,GChild,flag] = extendTree(tree,GChild,end_node,segmentLength,r,world,flag_chk,dim)   % segmentLength: maximum stepsize,  r: neighbor radius
  ...
  while flag1==0
    ...
    Vect=Vect/norm(Vect);
%     new_point(dim+1:2*dim)=rand*Vect;
    if dim==2
        new_point(dim+1:2*dim)=[-1+(1-(-1))*rand, -1+(1-(-1))*rand];     % vmin=-1 vmax=1
    else
      ...
      size_near = size(near_idx,1);
        ...
        for i = 1:size_near                                                % choose parent node
            ...
            if  cost_near+0.01 < min_cost
                ...
                if collision_flag==0
                    if sum((trajend(1:dim)-new_node(1:dim)).*(trajend(1:dim)-new_node(1:dim)))<0.4                 % norm(trajend-new_node(1:2*dim))<0.5        
                        min_cost = cost_near;
        ...
        for j = 1:size(reduced_idx,1)
          ...
          rnewcost = min_cost + lcost;
            if near_cost > rnewcost + 0.01
                ...
                if  collision_flag==0 
                    if sum((trajend(1:dim)-new_tree(reduced_idx(j),1:dim)).*(trajend(1:dim)-new_tree(reduced_idx(j),1:dim)))<0.4    % norm(trajend-new_tree(reduced_idx(j),1:2*dim))<0.5
